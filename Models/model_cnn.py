...
class Frames_network(nn.Module):
    """docstring for SubgoalExtractor§."""

    def __init__(self, input_frame_num=12,output_num=12, mode='subgoal'):
        """
        mode = [subgoal/evaluator]
        """
        super(Frames_network, self).__init__()
        self.conv_1 = nn.Conv3d(input_frame_num, 32, (1,3,3), stride=(1, 2, 2))
        self.relu_1 = nn.ReLU(inplace=True)
        self.conv_2 = nn.Conv3d(32, 64, (1,3,3), stride=1)
        self.instance_norm_1 = nn.InstanceNorm3d(64)
        self.relu_2 = nn.ReLU(inplace=True)
        self.conv_3 = nn.Conv3d(64, 128, (1,3,3), stride=(1,2,2))
        self.relu_3 = nn.ReLU(inplace=True)
        self.conv_4 = nn.Conv3d(128, 128, (1,3,3), stride=(1,1,1))
        self.instance_norm_2 = nn.InstanceNorm3d(128)
        self.relu_4 = nn.ReLU(inplace=True)
        self.conv_5 = nn.Conv3d(128, 256, (3,2,2), stride=(1,1,1))
        self.relu_5 = nn.ReLU(inplace=True)
        self.global_avg_pooling = nn.AdaptiveAvgPool3d((1,1,1))
        self.linear_1 = nn.Linear(256,512)
        self.linear_2 = nn.Linear(512,output_num)
...
class Predictor(nn.Module):
    ...
    def __init__(self):
        ...
        self.feature_network = FeatureExtractor()
        self.head = Detector_Head(mode='evaluator')
    def forward(self,x,length):
        out = self.feature_network(x)
        return self.head(out,length)
